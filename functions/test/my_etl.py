...
from functions.etl.abstract.etl import Etl
...
class MyEtl(Etl):
    ...
    def usable_products(self, ready_raw_materials: typing.Sequence[Product]) -> (
        ...
        return [x for x in ready_raw_materials if x.name == 'product1' and x.function.name == 'myoperator1'] + \
        [x for x in ready_raw_materials if x.name == 'product2' and x.function.name == 'myoperator2']
