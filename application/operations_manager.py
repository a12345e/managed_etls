...
class OperationsManager(ABC):
    """
    Initialize:
        create all functions objects, etl ones and sensors
            It is the shift of the principal to take place implementing the principal policy,
            and the principal must never
            start any new functions outside the shift time by means of the real time clock. So if the shift does not
            ...
            2) Check for zombie functions, namely operations that show death signs and had started at previous shift.
